...
function Board()
{
    ...
    return Component::create("div", [], [Component::create("header", ["className" => "app-header", "data-testid" => "app-header"], [Component::create("h1", [], ["TaskBoard Pro"])]), Component::create("div", ["className" => "kanban-board", "data-testid" => "kanban-board"], [Component::create("Column", ["id" => "todo", "title" => "To Do", "cards" => $cards['todo'] ?? [], "onAddCard" => fn($cardData) => $addCard('todo', $cardData), "onDeleteCard" => fn($cardId) => $deleteCard('todo', $cardId), "onUpdateCard" => fn($cardId, $updates) => $updateCard('todo', $cardId, $updates), "onDragStart" => fn($card) => $handleDragStart($card, 'todo'), "onDragEnd" => $handleDragEnd, "onDrop" => fn() => $handleDrop('todo')], []), Component::create("Column", ["id" => "in-progress", "title" => "In Progress", "cards" => $cards['in-progress'] ?? [], "onAddCard" => fn($cardData) => $addCard('in-progress', $cardData), "onDeleteCard" => fn($cardId) => $deleteCard('in-progress', $cardId), "onUpdateCard" => fn($cardId, $updates) => $updateCard('in-progress', $cardId, $updates), "onDragStart" => fn($card) => $handleDragStart($card, 'in-progress'), "onDragEnd" => $handleDragEnd, "onDrop" => fn() => $handleDrop('in-progress')], []), Component::create("Column", ["id" => "done", "title" => "Done", "cards" => $cards['done'] ?? [], "onAddCard" => fn($cardData) => $addCard('done', $cardData), "onDeleteCard" => fn($cardId) => $deleteCard('done', $cardId), "onUpdateCard" => fn($cardId, $updates) => $updateCard('done', $cardId, $updates), "onDragStart" => fn($card) => $handleDragStart($card, 'done'), "onDragEnd" => $handleDragEnd, "onDrop" => fn() => $handleDrop('done')], [])])]);
}