...
final class LZ4 {
     ...
     */
    private function calculateHash(int $sequence): int {
        $product = $this->imul($sequence, self::HASHER);

        return $this->unsignedRightShift($product, self::HASH_SHIFT); // Direct use approach (matching JS more closely)
    }
    ...
    private function imul(int $a, int $b): int {
      ...
      */
     private function unsignedRightShift(int $a, int $n): int {
         if ($n === 0) {
            return $a & 0xFFFFFFFF; // Return as unsigned 32-bit
         }
    ...
    public function compress(string $input): string {
        ...
        $outputBound = $this->compressBound($inputLength);
        ...
        $compressedSize = $this->compressBlock($inputBytes, $outputString, 0, 0, $inputLength);
      ...
      *
      * @param array<int> $src Source byte array.
     ...
     private function compressBlock(array $src, string &$dst, int $pos, int $dpos, int $srcLength): int {
         ...
         if ($srcLength > self::MF_LIMIT) {
             ...
             while ($pos < $srcLengthLimit) { // Match JS loop condition
               if ($pos + 3 >= $srcLengthLimit) {
                    break; // Need 4 bytes for JS hash sequence, check against limit
               }
               ...
               $hashIndex = $this->calculateHash($sequence); // Use the helper function
     ...
     */
    public function compressBound(int $size): int {
     ...
     */
    public function decompress(string $input, int $outputSize): string {
        ...
        while ($inputPos < $inputLength) {
            ...
            if ($literals_length === self::RUN_MASK) { // 15 requires reading more bytes
                ...
                while ($len === 255) {
                    if ($inputPos >= $inputLength) throw new RuntimeException(
                        "Malformed LZ4 stream: Unexpected end while reading literal length."
                    );
            ...
            if ($literals_length > 0) {
                ...
                if ($literalEndPos > $outputSize) throw new RuntimeException(
                    "Malformed LZ4 stream: Literal copy exceeds output buffer size."
                );
                $literalInputEndPos = $inputPos + $literals_length;

                 if ($literalInputEndPos > $inputLength) throw new RuntimeException(
                     "Malformed LZ4 stream: Literal copy exceeds input buffer."
                 );
            ...
            if ($inputPos + 1 >= $inputLength) throw new RuntimeException(
                "Malformed LZ4 stream: Unexpected end while reading offset."
            );
            $offset = $inputBytes[$inputPos++] | ($inputBytes[$inputPos++] << 8);

            if ($offset === 0 || $offset > $outputPos) {
                 throw new RuntimeException(
                     "Malformed LZ4 stream: Invalid offset " . $offset . " at output position " . $outputPos
                 );
            }
            ...
            if ($match_length === self::ML_MASK) { // 15 requires reading more bytes
                ...
                while ($len === 255) {
                     if ($inputPos >= $inputLength) throw new RuntimeException(
                         "Malformed LZ4 stream: Unexpected end while reading match length."
                     );
                     $len = $inputBytes[$inputPos++];
                     $match_length += $len;
                }
            ...
            if ($matchEnd > $outputSize) throw new RuntimeException(
                "Malformed LZ4 stream: Match copy exceeds output buffer size."
            );
        ...
        if ($outputPos !== $outputSize) {
            throw new RuntimeException(
                "Decompression failed: Expected output size {$outputSize}, but got {$outputPos}. Input exhausted: " . ($inputPos >= $inputLength ? 'yes' : 'no')
            );
        }
     ...
     */
    public function compressPackage(string $data, bool $verify = false): array {
        ...
        while ($offset < $dataLength) {
            ...
            $bound = $this->compressBound(strlen($chunk));
            ...
            if (strlen($compressed) > 0) {
                ...
                if ($verify) {
                    $decompressed = $this->decompress($compressed, strlen($chunk));
                    ...
                    for ($i = 0; $i < strlen($chunk); $i++) {
                        if (ord($chunk[$i]) !== ord($decompressed[$i])) {
                            throw new RuntimeException(
                                "Verification failed: byte mismatch at offset $offset, position $i"
                            );
                        }
                    }
                }
        ...
        $compressedData = [
            'data' => str_repeat(
                "\0",
                $total + $chunkSize * 2
            ), // Allocate space for compressed data plus two cached chunks
            'cachedOffset' => $total,
            'cachedIndexes' => [-1, -1],
